// prettier-ignore
function encryptaSenha (senhaDigitada:string, usuario:string) {
    senhaDigitada = senhaDigitada.padEnd(15, ' ');
    let cripA = (usuario.charCodeAt(0) % 15);
    let cripB = (usuario.charCodeAt(usuario.trim().length-1) % 15);
    let senha2 = senhaDigitada.substr(6,1)+senhaDigitada.substr(14,1)+senhaDigitada.substr(7,1)+senhaDigitada.substr(5,1)
    ...
    let cripA1 = senha2.substr(cripA,1);
    let cripB1 = senha2.substr(cripB,1);
